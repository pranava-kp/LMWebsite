...
// FIX: Added useLocation
import { useNavigate, useLocation } from "react-router-dom"; 
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
import rnsLogo from "../../../../assets/images/rns-logo.webp";
// FIX: Imported the new edit API
import { createLeave, editUserLeave } from "../../../../services/operations/leaveAPI"; 
...
const NewLeave = () => {
    ...
    useEffect(() => {
        ...
        if (location.state?.editLeaveData && staffList.length > 0 && !editLeaveId) {
            const data = location.state.editLeaveData;
            setEditLeaveId(data._id);

            setFormData({
                subject: data.subject,
                body: data.body,
                category: ["Casual Leave", "Earned Leave", "Maternity Leave", "Restricted Holiday"].includes(data.category) ? data.category : "Others",
                ...
                if (curr.getDay() !== 0) {
                    ...
                    if (dayDataFromDB && Object.keys(dayDataFromDB).length > 0) {
                        const periods = Object.keys(dayDataFromDB).map(hour => {
                            const staffId = dayDataFromDB[hour];
                            const staffObj = staffList.find(s => s._id === staffId);
                            return {
                                hour: hour,
                                substituteId: staffId,
                                substitute: staffObj ? formatStaffName(staffObj) : "Unknown Teacher"
                            };
                        });
                        mappedSubs[dateStr] = { hasClass: "yes", periods };
                    } else {
                        mappedSubs[dateStr] = { hasClass: "no", periods: [] };
                    }
                }
                curr.setDate(curr.getDate() + 1);
            }
            setSubstituteTeachers(mappedSubs);
        }
    }, [location.state, staffList, editLeaveId]);

    // Initialize daily schedule based on date range (Safeguarded for edit mode)
    useEffect(() => {
        if (startDateObj && endDateObj && startDateObj <= endDateObj) {
            ...
            setSubstituteTeachers((prev) => {
                ...
                dates.forEach((dateStr) => {
                    // This perfectly preserves the edit data if it was just loaded!
    ...
    const handlePeriodChange = (dateStr, index, field, value, staffId = null) => {
        setSubstituteTeachers((prev) => {
            ...
            return {
                ...prev,
                [dateStr]: { ...prev[dateStr], periods: updatedPeriods }
            };
        });
    };
    ...
    const handleOnSubmit = async (e) => {
        ...
        try {
            ...
            // FIX: If we are in edit mode, append the ID!
            if (editLeaveId) {
                formDataToSend.append("leaveId", editLeaveId);
            }
            ...
            // FIX: Switch between Create and Edit API routes
            let result;
            if (editLeaveId) {
                result = await editUserLeave(formDataToSend, token); 
            } else {
                result = await dispatch(createLeave(formDataToSend, token));
            }
        ...
        <div className="p-10 bg-gray-50 min-h-screen">
            <div className="max-w-5xl mx-auto bg-white shadow-xl rounded-2xl p-8">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-blue-600">
                            {editLeaveId ? "Edit Leave Request" : "Leave Application"}
                        </h1>
                        <p className="text-sm text-gray-500">
                            {editLeaveId ? "Modify your pending request" : "Submit a new leave request"}
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleOnSubmit} className="flex flex-col gap-6">
                    {/* SUBJECT */}
                    <div>
                        <label className="block text-sm font-semibold mb-2 uppercase">Subject *</label>
                        <input type="text" name="subject" value={formData.subject} onChange={handleOnChange} placeholder="Enter subject" className="w-full border rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-300 outline-none" required autoComplete="off" />
                    </div>

                    {/* REASON */}
                    <div>
                        <label className="block text-sm font-semibold mb-2 uppercase">Detailed Reason *</label>
                        <textarea name="body" value={formData.body} onChange={handleOnChange} rows="5" placeholder="Explain your leave reason" className="w-full border rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-300 outline-none" required />
                    </div>

                    {/* DATE + TYPE */}
                    <div className="grid grid-cols-3 gap-6">
                        <div>
                            <label className="text-sm font-semibold uppercase mb-2 block">From *</label>
                            <DatePicker selected={startDateObj} onChange={(date) => { setStartDateObj(date); setFormData((prev) => ({ ...prev, startDate: formatForApi(date) })); const maxDate = getMaxEndDate(date); if (date && (!endDateObj || date > endDateObj || (maxDate && endDateObj > maxDate))) { setEndDateObj(date); setFormData((prev) => ({ ...prev, endDate: formatForApi(date) })); } }} dateFormat="dd/MM/yyyy" placeholderText="dd/mm/yyyy" minDate={getYesterday()} filterDate={isNotSunday} className="w-full border rounded-xl px-3 py-2" required />
                        </div>
                        <div>
                            <label className="text-sm font-semibold uppercase mb-2 block">To *</label>
                            <DatePicker selected={endDateObj} onChange={(date) => { setEndDateObj(date); setFormData((prev) => ({ ...prev, endDate: formatForApi(date) })); }} dateFormat="dd/MM/yyyy" placeholderText="dd/mm/yyyy" minDate={startDateObj || getYesterday()} maxDate={startDateObj ? getMaxEndDate(startDateObj) : null} filterDate={isNotSunday} className="w-full border rounded-xl px-3 py-2" required />
                        </div>
                        <div>
                            <label className="text-sm font-semibold uppercase mb-2 block">Leave Type *</label>
                            <select name="category" value={formData.category} onChange={handleOnChange} className="w-full border rounded-xl px-3 py-2" required>
                            ...
                            {formData.category === "Others" && (
                                <input type="text" name="otherCategory" value={formData.otherCategory} onChange={handleOnChange} placeholder="Specify leave type" className="w-full border rounded-xl px-3 py-2 mt-2" required />
                            )}
                        </div>
                    </div>
                                ...
                                <div key={dateStr} className="bg-gray-50 border rounded-xl p-4 mb-4">
                                    <div className="flex items-center gap-6 mb-3">
                                        ...
                                        <div className="flex gap-4">
                                            <label className="flex items-center gap-1"><input type="radio" checked={substituteTeachers[dateStr].hasClass === "yes"} onChange={() => handleClassRadio(dateStr, "yes")} /> Yes</label>
                                            <label className="flex items-center gap-1"><input type="radio" checked={substituteTeachers[dateStr].hasClass === "no"} onChange={() => handleClassRadio(dateStr, "no")} /> No</label>
                                        </div>
                                    </div>
                                        ...
                                        <div className="flex flex-col gap-3 ml-2">
                                            {substituteTeachers[dateStr].periods.map((period, idx) => (
                                                <div key={idx} className="flex gap-3 relative items-center">
                                                    <input type="text" placeholder="Hour (e.g., 1)" className="border rounded px-3 py-2 w-1/3" value={period.hour} onChange={(e) => handlePeriodChange(dateStr, idx, "hour", e.target.value)} required />
                                                    <div className="relative w-2/3">
                                                        <input type="text" placeholder="Select Substitute Teacher" className={`border rounded px-3 py-2 w-full ${!period.substituteId && period.substitute.length > 0 ? 'border-red-400' : ''}`} value={period.substitute} onChange={(e) => { handlePeriodChange(dateStr, idx, "substitute", e.target.value); setActiveDropdown(`${dateStr}-${idx}`); }} onFocus={() => setActiveDropdown(`${dateStr}-${idx}`)} onBlur={() => setTimeout(() => setActiveDropdown(null), 200)} required />
                                                        {activeDropdown === `${dateStr}-${idx}` && period.substitute.length > 0 && (
                                                            <div className="absolute z-10 w-full bg-white border border-gray-300 rounded mt-1 max-h-48 overflow-y-auto shadow-lg">
                                                                {staffList.map(staff => ({ name: formatStaffName(staff), id: staff._id })).filter(staff => staff.name.toLowerCase().includes(period.substitute.toLowerCase())).map((staff, sIdx) => (
                                                                    <div key={sIdx} className="px-3 py-2 cursor-pointer hover:bg-gray-100 text-sm" onMouseDown={(e) => { e.preventDefault(); handlePeriodChange(dateStr, idx, "substitute", staff.name, staff.id); setActiveDropdown(null); }}>{staff.name}</div>
                                                                ))}
                                                    ...
                                                    {substituteTeachers[dateStr].periods.length > 1 && (
                                                        <button type="button" onClick={() => handleRemovePeriod(dateStr, idx)} className="text-red-500 text-sm font-semibold ml-2 whitespace-nowrap">Remove</button>
                                                    )}
                                                </div>
                                            ))}
                                            <button type="button" onClick={() => handleAddPeriod(dateStr)} className="bg-gray-200 px-3 py-1 rounded text-sm w-max mt-1">+ Add Another Class</button>
                                        </div>
                                    )}
                                </div>
                            ))}
                    ...
                    <div className="flex justify-center pt-6">
                        ...
                        >
                            {loading ? "Processing..." : (editLeaveId ? "Save Changes" : "Submit")}
                        </button>
                    </div>
                </form>
            </div>
